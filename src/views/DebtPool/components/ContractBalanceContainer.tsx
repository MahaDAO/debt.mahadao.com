...
import { BNHUNDRED, DECIMALS_18, LOADING_DEFAULT_BASIC_STATE } from "../../../utils/constants";
...
const ContractBalanceContainer = () => {
  ...
  const percentOfDebtBurned = useMemo(() => {
    ...
    if(totalSupply.value !== totalCirculatingSupply.value && !totalSupply.value.isZero()){
      ...
      let diff = totalCirculatingSupply?.value.sub(totalSupply.value)
      let val = BNHUNDRED.sub(totalCirculatingSupply.value.sub(diff).mul(100).div(totalCirculatingSupply.value)).toNumber()
  ...
  const totalDebtBurned = useMemo(() => { 
    ...
    if(totalSupply.value !== totalCirculatingSupply.value && !totalSupply.value.isZero()){
      const val = totalCirculatingSupply.value.sub(totalSupply.value)
      return {
        isLoading: false,
        value: val
      }
        ...
        <div className="m-b-8">
          {
            totalDebtBurned.isLoading
              ...
              : (
                <TextWrapper
                  text={`$ ${
                    Number(getDisplayBalance(totalDebtBurned.value, 18, 3)).toLocaleString(undefined, { minimumFractionDigits: 3 })
                  }`}
                  fontSize={24}
                  fontWeight={'bold'}
                  Fcolor={'#FFFFFF'}
      ...
      <div className="material-primary m-b-16 single-line-center-center" style={{flex: 1}}>
        <div className="m-b-8">
          ...
          <div className="single-line-center-center m-b-16">
            <TextWrapper fontSize={24} text={'Total pending payback'} Fcolor={theme.color.transparent[100]} />
          </div>
        </div>
